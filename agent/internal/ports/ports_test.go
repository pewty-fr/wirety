...
// mockCaptivePortal implements CaptivePortalPort for testing
type mockCaptivePortal struct {
	running          bool
	nonAgentPeers    []string
	whitelistedPeers []string
}

func (m *mockCaptivePortal) Start() error {
	m.running = true
	return nil
}

func (m *mockCaptivePortal) Stop() error {
	m.running = false
	return nil
}

func (m *mockCaptivePortal) UpdateNonAgentPeers(peerIPs []string) {
	m.nonAgentPeers = peerIPs
}

func (m *mockCaptivePortal) AddWhitelistedPeer(ip string) {
	m.whitelistedPeers = append(m.whitelistedPeers, ip)
}

func (m *mockCaptivePortal) RemoveWhitelistedPeer(ip string) {
	for i, peer := range m.whitelistedPeers {
		if peer == ip {
			m.whitelistedPeers = append(m.whitelistedPeers[:i], m.whitelistedPeers[i+1:]...)
			break
		}
	}
}

func (m *mockCaptivePortal) ClearWhitelist() {
	m.whitelistedPeers = []string{}
}

// mockTLSSNIGateway implements TLSSNIGatewayPort for testing
type mockTLSSNIGateway struct {
	running          bool
	nonAgentPeers    []string
	whitelistedPeers []string
...
func (m *mockTLSSNIGateway) Start() error {
	m.running = true
	return nil
}

func (m *mockTLSSNIGateway) Stop() error {
	m.running = false
	return nil
}

func (m *mockTLSSNIGateway) UpdateNonAgentPeers(peerIPs []string) {
	m.nonAgentPeers = peerIPs
}

func (m *mockTLSSNIGateway) AddWhitelistedPeer(ip string) {
	m.whitelistedPeers = append(m.whitelistedPeers, ip)
}

func (m *mockTLSSNIGateway) RemoveWhitelistedPeer(ip string) {
	for i, peer := range m.whitelistedPeers {
		if peer == ip {
			m.whitelistedPeers = append(m.whitelistedPeers[:i], m.whitelistedPeers[i+1:]...)
			break
...
func (m *mockTLSSNIGateway) ClearWhitelist() {
	m.whitelistedPeers = []string{}
}

func (m *mockTLSSNIGateway) AddAllowedDomain(domain string) {
